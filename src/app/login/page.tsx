...
function LoginContent() {
  ...
  return (
    // DIV FORM: Warna putih bersih dengan shadow tebal biar misah dari background
    <div className="bg-white p-8 sm:p-12 rounded-[2rem] w-full max-w-md shadow-2xl shadow-blue-200/40 border border-blue-100 relative z-10 animate-in zoom-in-95 duration-500">
      ...
      <div className="text-center mb-10 flex flex-col items-center">
        ...
        <p className="text-slate-500 font-bold text-xs uppercase tracking-widest mt-2">Login to Continue</p>
      </div>
      ...
      {error && (
        <div className="bg-red-50 text-red-600 p-4 rounded-2xl text-xs font-bold mb-6 text-center border border-red-100 animate-in slide-in-from-top-2">
          {error === 'not_found' ? "❌ Akun tidak ditemukan. Daftar dulu yuk!" : "🔑 Password salah. Coba ingat lagi."}
        </div>
      )}
      ...
      <form action={login} className="space-y-5">
        ...
        <div className="space-y-2">
          <label className="text-[11px] font-bold text-slate-500 uppercase tracking-widest ml-3">Password</label>
          <input 
            name="password"
            type="password" 
            placeholder="••••••••" 
        ...
        <button className="w-full py-4 mt-4 bg-blue-600 text-white rounded-2xl font-black hover:bg-blue-700 transition-all duration-200 shadow-lg shadow-blue-200 hover:shadow-blue-300 flex items-center justify-center gap-2 active:scale-95">
            <LogIn size={20} />
      ...
      <div className="mt-8 text-center">
        <p className="text-xs text-slate-500 font-medium mb-2">Belum punya akun?</p>
        <Link href="/register" className="text-sm font-black text-blue-600 hover:text-blue-800 transition-colors border-b-2 border-transparent hover:border-blue-600 pb-0.5">
          DAFTAR SEKARANG
        </Link>
      </div>
    </div>
...
export default function LoginPage() {
  return (
    // BACKGROUND HALAMAN: Dibuat warna biru keabu-abuan pucat biar form putihnya menonjol
    <main className="min-h-screen bg-[#F0F4F8] flex items-center justify-center p-6 font-sans relative overflow-hidden">
      
      {/* Sedikit tint biru tambahan biar suasananya tetep kerasa 'Xolva' */}
